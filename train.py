...
class TFLogger(object):
    ...
    def __init__(self, log_dir):
        ...
        self.writer = tf.summary.create_file_writer(log_dir)  # tf.summary()的各类方法，能够保存训练过程以及参数分布图并在tensorboard显示
...
class Trainer(Procedure):
    ...
    def _init_model(self):
        model_class = get_attr_by_name(self._args['--model-class']) # models.updater.CoAttnBPBAUpdater   存储的是模型
    ...
    def load_dataset(self):
        ...
        example_class = get_attr_by_name(self._args['--example-class'])   # 所谓的动态载入Example Class类 用来保存 Example
        train_set = Dataset.create_from_file(self._args['--train-data'], example_class)  # 读取JSON文件，Example封装了一系列的方法
        dev_set = Dataset.create_from_file(self._args['--dev-data'], example_class)   # 验证集合
        return train_set, dev_set

    def train(self):
        train_set, dev_set = self.load_dataset()  # 加载数据
        self._init_model()
...
def train(args):
    ...
    seed = int(args['--seed'])  # 种子数
    set_reproducibility(seed)  # 设置种子数 具有可复现
