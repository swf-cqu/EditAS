...
class BothAttnSeqEditor(BaseEditor):
    ...
    def step(self, y_tm1_embed: Tensor, static_input: Any, state_tm1: Tuple) -> Tuple[Tuple, Tuple]:
        ...
        # hidden_t: query vector;  src_encodings: value vector; src_encodings_att_linear: key vector
        ...
        out_vec = att_t   # 最后还要接一个Dense layer
        return state_tm1, (out_vec, src_alpha_t, edit_alpha_t)  # src_alpha_t, edit_alpha_t 会在后续的指针使用
...
class SrcPtrBASeqEditor(BothAttnSeqEditor):
    ...
    def cal_gen_words_prob(self, att_ves, max_ext_size) -> Tensor:
        ...
        if max_ext_size != 0:
            ext_zero_size = [*gen_words_prob.size()[:-1], max_ext_size]  # shape 扩充的单词添加到基本字典后的shape
